...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const wnusdBalance = useSelector<IReduxState, string>(state => {
        return state.account.balances && state.account.balances.wnusd;
    });
    ...
    const nusdAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.nusd;
    });
    ...
    const setMax = () => {
        if (isWrap) {
            setValue(nusdBalance);
        } else {
            setValue(wnusdBalance);
        }
    };
    ...
    const hasAllowance = useCallback(() => nusdAllowance > 0, [nusdAllowance]);

    const trimmedNusdBalance = trim(Number(nusdBalance), 6);
    const trimmedWnusdBalance = trim(Number(wnusdBalance), 6);

    const getBalance = () => (isWrap ? `${trimmedNusdBalance} NUSD` : `${trimmedWnusdBalance} wNUSD`);
            ...
            <Paper className="ohm-card ohm-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "NUSD" : "wNUSD"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wNUSD" : "NUSD"}`}
                        </p>
                    </div>
                </div>
