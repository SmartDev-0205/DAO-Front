...
import { StakingContract, NusdTokenContract, NmetaTokenContract } from "../../abi";
...
export const loadAppDetails = createAsyncThunk(
    ...
    async ({ networkID, provider }: ILoadAppDetails) => {
        ...
        const nmetaBondsAmountsPromises = allBonds.map(bond => bond.getNmetaAmount(networkID, provider));
        const nmetaBondsAmounts = await Promise.all(nmetaBondsAmountsPromises);
        const nmetaAmount = nmetaBondsAmounts.reduce((nmetaAmount0, nmetaAmount1) => nmetaAmount0 + nmetaAmount1, 0);
        const nmetaSupply = totalSupply - nmetaAmount;

        const rfv = rfvTreasury / nmetaSupply;

        const epoch = await stakingContract.epoch();
        const stakingReward = epoch.distribute;
        const circ = await nusdContract.circulatingSupply();
