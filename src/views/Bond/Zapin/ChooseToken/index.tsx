...
function ChooseToken({ open, handleClose, handleSelect, bond }: IChooseTokenProps) {
    ...
    const filtredTokens = tokens.filter(({ name, address, isBnb }) => {
        ...
        if (quantity && quantity.length === 42) {
            addressTest = address[chainID].toLocaleLowerCase() === quantity.toLowerCase();
        }
        ...
        if (bond.name === wbnb.name) {
            lpFilter = isBnb ? false : bnbToken.address !== address;
        }
    ...
    return (
        <Modal id="hades" open={open} onClose={handleClose} hideBackdrop>
            <Paper className="nmeta-card nmeta-popover choose-token-poper">
                ...
                <Box>
                    ...
                    <div className="choose-token-poper-body">
                        {filtredTokens.map(token => (
                            <div onClick={() => handleSelect(token)} key={token.address[chainID]} className="choose-token-poper-body-item">
